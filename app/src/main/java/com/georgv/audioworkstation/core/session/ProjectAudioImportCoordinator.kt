...
class ProjectAudioImportCoordinator @Inject constructor(
    ...
    /** Wait until [observedTracks] matches the repository snapshot for [projectId]. */
    suspend fun awaitObservedTracksSynced(
        projectId: String,
        observedTracks: StateFlow<List<TrackEntity>>,
    ) {
        val expectedTracks = repo.observeTracks(projectId).first()
        observedTracks.first { it == expectedTracks }
    }

    suspend fun recoverStaleImports(
        projectId: String,
        activeImportJobs: Map<String, Job>,
    ) {
        val stale =
            repo.observeTracks(projectId).first().filter { track ->
                track.importStatus == TrackImportStatus.IMPORTING &&
                    activeImportJobs[track.id]?.isActive != true
            }
